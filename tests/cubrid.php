...
try {
	...
	printf ("     If you want to use UTF-8, run CUBRID with 'export CUBRID_BYTES_PER_CHAR=1'\n");
	...
	$r = $db->query ('SELECT * FROM db_class WHERE class_name = ?', 's', 'edb_test');
	$db->free_result ();
	if ( $r == 1 ) {
